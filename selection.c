...
//*************************************************************
// Let's keep a list of LAGs for ease & debug.
//*************************************************************
...
// static int is_port_partner_port(const ulong, LAG_t *const);
static int is_port_partner_port(port_handle_t, LAG_t *const); // XXX
static void LAG_select_aggregator(LAG_t *const, lacp_per_port_variables_t *);
static void print_lag_id(LAG_Id_t *lag_id);


// XXX
// extern int is_pmask_zero(port_mask_t *); // XXX shd go away XXX
...
compare_lag_id (LAG_Id_t *first_lag_id, LAG_Id_t *second_lag_id)
{
  ...
  /***************************************************************************
   * Compare system Id (priority + mac addr), port key and 
...
int compare_port_handle(void* lag_port_struct1, void* lag_port_struct2)
{
...
/*1*/if (lag == NULL) {		/* This port does not belong to any LAG. */
...
#if 0 // XXX
///      FOR_ALL_LAGS(i) {
///          if (i == port || lacp_lags[i] == NULL ||
///              lacp_lags[i]->port_type != port_type ||
///              compare_lag_id(lacp_lags[i]->LAG_Id, lagId) == FALSE)
///              continue;
///
///          lag = lacp_lags[i];
///          break;
///      }
      ...
      // XXX Why check if "i == port" again ?? XXX
      ...
      {
        if (!plp->lag) continue;
        if (plp->lag->port_type != lacp_port->port_type) continue;
	...
	if (0 == (memcmp(plp->partner_oper_system_variables.system_mac_addr, 
			...
			MAC_ADDR_LENGTH))) continue;

        if (compare_lag_id(plp->lag->LAG_Id, lagId) == FALSE) continue;
...
/*2*/ if (lag == NULL) {
          char lag_id_str[80];
	  /*
           * No LAG found with the same LAG id.  Could be the first port to
           * join a new LAG or the only (individual) port to form an individual
           * LAG.
           */
          if ((lag = (LAG_t *)malloc(sizeof(LAG_t))) == NULL) {
              free(lagId);
              // XXX XXX ERR_errlog(ERR_FORCEFATAL, lacp_efacid, LACP_E_NOMEMLAG, 0);
          ...
          lag->LAG_Id = lagId;
          lag->loop_back = loop_back_check(lacp_port) ? TRUE : FALSE; // XXX

          // XXX MASK_SET(&lag->pmask, port);
          // XXX lacp_lags[port] = lag;

          //*************************************************************
          // The above 2 lines have been changed as follows XXX
          //*************************************************************
          // XXXXXX ?? XXXXXXX for now
          // MASK_SET(&lag->pmask, lacp_port->lport_handle);
          ...
          lag->pplist = n_list_insert_sorted(lag->pplist, 
                              (void *)plag_port_struct, compare_port_handle);
	     ...
	     LAG_id_string(lag_id_str, lagId);
          ...
          LAG_select_aggregator(lag, lacp_port); // XXX
	  lacp_port->selecting_lag = FALSE;
	  lacp_unlock(lock);
          return;
      }  /* (2) if (lag == NULL) */
      ...
      if (n_list_find_data(lag->pplist, 
          ...
          if ((lag->loop_back = loop_back_check(lacp_port)) == FALSE &&
              ...
              LAG_id_string(lag_id_str, lagId);
              // XXX ERR_errlog(0, lacp_efacid, LACP_I_PORTADDED, 2,
                         // Ports[port]->Port_Name, lag_id_str);
              if (lacp_port->debug_level & DBG_SELECT) {
      ...
      // XXX assert(0);	/* How could it be part of some LAG?! */
   ...
   */
  // XXXXXX for now XXXXXX
  // if (MASK_ISSET(&lag->pmask, lacp_port->lport_handle) && 
     pdummy = n_list_find_data(lag->pplist, 
                           &lacp_lag_port_match, 
                           &lacp_port->lport_handle);
     if (pdummy != NULL) plag_port_struct = pdummy->data;
     
     //*****************************************************************
     // XXX Removed lacp_enabled check below XXX
     //*****************************************************************
     if ( plag_port_struct && 
	  ( ((lag->loop_back = loop_back_check(lacp_port)) == TRUE) || // XXX
       ...
       */
       ...
       LACP_mux_fsm(E2, lacp_port->mux_fsm_state, lacp_port); // XXX

       lacp_port->lacp_control.ready_n = FALSE;
       // XXXXXX
       // MASK_CLR(&lag->pmask, lacp_port->lport_handle); // XXX
       lag->pplist =  n_list_remove_data(lag->pplist, plag_port_struct);

       LAG_id_string(lag_id_str, lag->LAG_Id);
       // XXX ERR_errlog(0, lacp_efacid, LACP_I_PORTREMOVED, 2,
       // Ports[port]->Port_Name, lag_id_str);
       if (lacp_port->debug_level & DBG_SELECT) {
      ...
      // if (is_pmask_zero(&lag->pmask))
       if (lag->pplist == NULL) { // XXX
         /*
      ...
      // XXX if (lacp_port->lacp_control.lacp_enabled == TRUE)
       LAG_selection(lacp_port); // XXX Unconditional for us
...
{
    // lacp_per_port_variables_t *lacp_port = &lacp_per_port_vars_array[port];
    ...
    if (mlacp_blocking_send_select_aggregator(lag,lacp_port) == R_SUCCESS) {
      ...
      lag->sp_handle = lacp_port->sport_handle; // Halon: Save handle for clearing later.
      LACP_mux_fsm(E1, lacp_port->mux_fsm_state, lacp_port); // XXX
    }
...
//******************************************************************
// Function : LAG_attached_to__aggregator
// Unused ?
//******************************************************************
void
LAG_attached_to_aggregator(port_handle_t lport_handle,int result)
{
    ...
    if(result == R_SUCCESS) { 
      ...
      lacp_port->lacp_control.selected = SELECTED;
      LACP_mux_fsm(E1, lacp_port->mux_fsm_state, lacp_port);
    }
 ...
 */
static int
is_port_partner_port(port_handle_t lport_handle, LAG_t *const lag) // XXX
{
    // XXX int p;
    ...
    // if (!lag || is_pmask_zero(&lag->pmask))
    if (!lag || lag->pplist == NULL)
        return 0;
    
#if 0 // XXX change the logic XXX
    FOR_ALL_PORTS(p) {
        if (!MASK_ISSET(&lag->pmask, p))
            continue;

        if (lacp_per_port_vars_array[port].partner_oper_port_number == p)
            return 1;
    }
#endif //0 // XXX
    ...
    {
         // XXXXXX ?? XXXXXX for now ?? 
         // if (!MASK_ISSET(&lag->pmask, plpinfo->lport_handle)) continue;
         if (n_list_find_data(lag->pplist, 
                              &lacp_lag_port_match, 
                              &lport_handle) == NULL) continue;

         // XXXXXX do we need phy port Id ?? XXXXXX
...
form_lag_id(lacp_per_port_variables_t *lacp_port)
{
  ...
  if( !(lagId = (LAG_Id_t *)malloc(sizeof(LAG_Id_t)))) {
    // XXX ERR_errlog(ERR_FORCEFATAL, lacp_efacid, LACP_E_NOMEMLAGID, 0);

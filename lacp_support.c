 ...
 *----------------------------------------------------------------------*/
void
LACPResetProtocol(lacp_per_port_variables_t *plpinfo) // XXX
{
    RENTRY();
                     ...
                     int hw_collecting)
{
  ...
  //          on this port, just kill it and restart with the
  //          latest config info.
  if (plpinfo != NULL) {
...

  {
    ...
    /********************************************************************
     *  Set the state machines in the BEGIN state initially. XXX moved XXX
     ********************************************************************/
 ...
 *
 * Synopsis: Called whenever a port's dynamicaly changeable LACP parameter
 *           is modified.
 *----------------------------------------------------------------------*/
void
                              ...
                              int hw_collecting)
{
  ...
  /***************************************************************************
   *     set the misc variables  // XXX Added
   ***************************************************************************/
  plpinfo->collector_max_delay = htons(DEFAULT_COLLECTOR_MAX_DELAY);


  /***************************************************************************
   *  Enable the port(for lacp), LACP on the port, put port_moved to false
   ***************************************************************************/
...
LACP_disable_lacp(port_handle_t lport_handle)
{
  ...
  //****************************************************************
  // As LACP is per-port, we go ahead & de-register the MAC for this port.
  //****************************************************************
  deregister_mcast_addr(plpinfo->lport_handle);
...
LACP_reset_port(port_handle_t lport_handle)
{
  ...
  // XXX lacp_port->lacp_control.lacp_enabled = FALSE;
      ...
      (plpinfo->partner_oper_port_state.lacp_activity == LACP_PASSIVE_MODE))
  {
    LACP_periodic_tx_fsm(E8,
      ...
      if ((plpinfo->lacp_control.begin == TRUE) ||
          // XXX (plpinfo->lacp_control.lacp_enabled == FALSE) ||
          (plpinfo->lacp_control.port_enabled == FALSE))
       {
...
#if 0 // XXX
    RDBG("      LACP_Enabled:      %s\n",
           lacp_port->lacp_control.lacp_enabled ? "TRUE" : "FALSE");
#endif //0
...
print_lacp_key_group(const int key)
{
    ...
    {
        // XXX if (plpinfo->lacp_control.lacp_enabled == FALSE) continue;
...
#if 0 // XXX
    if (lacp_port->lacp_control.lacp_enabled == FALSE) {
        RDEBUG(DL_ERROR, "lacp not enabled on lport_handle 0x%llx\n",
...
get_lacp_params(struct MLt_lacp_api__lport_params *pMsg, int getnext)
{
    ...
    pMsg->actor_admin_port_priority =
        ntohs(lacp_port->actor_admin_port_priority);

    pMsg->actor_oper_port_priority =
    ...
    pMsg->actor_synchronization =
         lacp_port->actor_oper_port_state.synchronization;

    pMsg->actor_collecting =
         lacp_port->actor_oper_port_state.collecting;

    pMsg->actor_distributing =
         lacp_port->actor_oper_port_state.distributing;

    pMsg->actor_defaulted =
         lacp_port->actor_oper_port_state.defaulted;

    pMsg->actor_expired =
         lacp_port->actor_oper_port_state.expired;
    ...
    pMsg->partner_oper_port_number =
         ntohs(lacp_port->partner_oper_port_number);
    pMsg->partner_oper_port_priority =
         ntohs(lacp_port->partner_oper_port_priority);

    pMsg->partner_activity =
      lacp_port->partner_oper_port_state.lacp_activity;

    pMsg->partner_timeout =
      lacp_port->partner_oper_port_state.lacp_timeout;

    pMsg->partner_aggregation =
      lacp_port->partner_oper_port_state.aggregation;

    pMsg->partner_synchronization =
      lacp_port->partner_oper_port_state.synchronization;

    pMsg->partner_collecting =
      lacp_port->partner_oper_port_state.collecting;

    pMsg->partner_distributing =
      lacp_port->partner_oper_port_state.distributing;

    pMsg->partner_defaulted =
      lacp_port->partner_oper_port_state.defaulted;

    pMsg->partner_expired =
      lacp_port->partner_oper_port_state.expired;
}
	       ...
	       int type)
{
  ...
  L2_hexmac_to_strmac((u8_t*)dst_mac,
		      buffer,
		      sizeof(buffer),
		      L2_MAC_TWOxSIX);
  ...
  L2_hexmac_to_strmac((u8_t*)src_mac,
		      buffer,
		      sizeof(buffer),
		      L2_MAC_TWOxSIX);
  ...
  // XXX printf("Actor state : 0x%x\n", lacpdu_payload->actor_state);
  ...
  // printf("Partner state : 0x%x\n", (unsigned char) lacpdu_payload->partner_state); XXX
                             ...
                             int type)
{
  ...
  // printf("Requestor Transaction ID: %d\n",
         // ntohl(marker_response_payload->requester_transaction_id));
  printf("TLV type, Terminator: %d\n",
 ...
 */
int
LACP_waiting_link_up(port_handle_t lport_handle) // XXX
{
    // XXX lacp_per_port_variables_t *lacp_port = &lacp_per_port_vars_array[port];
    lacp_per_port_variables_t *lacp_port;

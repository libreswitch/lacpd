...
{
   NList *pval;
    /* TODO: make this into a chunk allocator */

   //Halon:  skip NEMO_MALLOC due to n_assert not compiling correctly.
...
n_list_insert_list_after(NList *prevelem, NList *list_to_insert)
{
    ...
    if(prevelem == NULL)
        return 0; // so where do you insert?

    if(list_to_insert == NULL)
        return 1; // done inserting an empty list. quick, eh?

    // both the list being inserted into and the list being inserted contain
    // atleast one node
...
n_list_prepend(NList *list, void *data)
{
    NList *elem = n_list_alloc();
    if (!elem)
        return NULL; // TODO: bad interface
    if (!list) {
        elem->next = elem;
        elem->prev = elem;
        elem->data = data;

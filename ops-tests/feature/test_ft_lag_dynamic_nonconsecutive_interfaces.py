...
###############################################################################
# Name         test_ft_lag_dynamic_nonconsecutive_interfaces.py
#
# Objective:  Verify that a dynamic LAG can be configured using
#             non-consecutive interfaces
#
# Description: This test verifies that a dynamic LAG can be configured using
#               non-consecutive interfaces.
#
...
def test_dynamic_nonconsecutive_interfaces(topology, step):
    ...
    step("#### Associate nonconsecutives interfaces LAG ####")
    for port in ports_sw1[::2]:
        associate_interface_to_lag(sw1, port, lag_id)

    for port in ports_sw2[::2]:
        associate_interface_to_lag(sw2, port, lag_id)

    step("#### Verify LAG configuration ####")
    verify_lag_config(sw1, lag_id, ports_sw1[::2], mode=mode_active)
    verify_lag_config(sw2, lag_id, ports_sw2[::2], mode=mode_passive)
    ...
    verify_state_sync_lag(sw1, ports_sw1[::2], LOCAL_STATE, mode_active)
    verify_state_sync_lag(sw1, ports_sw1[::2], REMOTE_STATE, mode_passive)

...
#              and a passive member can be deleted when it has 2, 1 or
#              0 members.
#
...
def test_dynamic_minimum_members(topology, step):
    ...
    step("##### Create LAGs with one interface ####")
    create_lag(sw1, lag_id, mode_active)
    create_lag(sw2, lag_id, mode_passive)
    ...
    step("#### Verify LAG configuration ####")
    verify_lag_config(sw1, lag_id, ports_sw1[1], mode=mode_active)
    verify_lag_config(sw2, lag_id, ports_sw2[1], mode=mode_passive)
    ...
    step("### Verify if LAG is synchronized")
    verify_state_sync_lag(sw1, ports_sw1[1], LOCAL_STATE, mode_active)
    verify_state_sync_lag(sw1, ports_sw1[1], REMOTE_STATE, mode_passive)
